...
// In production, DATABASE_URL should point at a persistent database (e.g. a
// Turso libSQL database, "libsql://...") so accounts and characters survive
// deploys and restarts — Render's own filesystem is wiped on every deploy.
// Locally (and if DATABASE_URL isn't set) it falls back to a SQLite file on
...
export async function initDb() {
  ...
  // `users` table had `email TEXT UNIQUE NOT NULL`; SQLite/libSQL can't drop a
  // NOT NULL constraint via ALTER, so the table is rebuilt with the new shape
  // instead.
  try {
    const columns = (await db.execute(`PRAGMA table_info(users)`)).rows as unknown as { name: string }[];
  ...
  try {
    await db.execute(`ALTER TABLE characters ADD COLUMN quidditch_position TEXT`);
  } catch {
    // column already exists
  }
  try {
    await db.execute(`ALTER TABLE characters ADD COLUMN story_flags TEXT NOT NULL DEFAULT '{}'`);
  } catch {
    // column already exists
  }
}
